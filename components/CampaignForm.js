...
export default function CampaignForm({ onAdd, initialData, triggerLabel = "Create Campaign" }) {
  ...
  const [form, setForm] = useState({ name: "", budget: "", status: "Active" });
  ...
  const handleSave = () => {
    ...
    setOpen(false);
    setForm({ name: "", budget: "", status: "Active" });
  };

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className='bg-[#2EC773] hover:bg-gray-800'>{triggerLabel}</Button>
      </DialogTrigger>
